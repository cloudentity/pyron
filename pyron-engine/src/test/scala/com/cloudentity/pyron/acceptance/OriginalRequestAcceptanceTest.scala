...
class OriginalRequestAcceptanceTest extends PyronAcceptanceTest with MustMatchers with MockUtils {
  ...
  def shouldSetTargetRequestOriginalPathParamsAndQueryParamsAndMethod(): Unit = {
    ...
    val original = OriginalRequest(HttpMethod.GET, UriPath(relativeUri.path), QueryParams("queryParam1" -> List("queryValue1")), Headers(), Nil, Some(Buffer.buffer()), PathParams(Map("param1" -> "value1")))
  ...
  def shouldSetTargetRequestOriginalBody(): Unit = {
      ...
      val body = """test-body"""
      val original = OriginalRequest(HttpMethod.POST, UriPath(path), QueryParams.empty, Headers(), Nil, Some(Buffer.buffer(body)), PathParams.empty)
