...
import com.cloudentity.edge.domain.flow.{EndpointMatchCriteria, PathMatching, PathPrefix, PluginConf, PluginName, RequestCtx, ResponseCtx, StaticServiceRule, TargetHost}
import com.cloudentity.edge.domain.rule.{RequestPluginsConf, ResponsePluginsConf, RuleConf, RuleConfWithPlugins}
import com.cloudentity.edge.plugin.verticle.{RequestPluginVerticle, ResponsePluginVerticle}
...
class RuleBuilderSpec extends WordSpec with MustMatchers with VertxSpec {
  ...
  "RuleBuilder.build" should {
    val ruleConf = RuleConf(None, EndpointMatchCriteria(HttpMethod.GET, PathMatching("".r, Nil, PathPrefix(""), "")), StaticServiceRule(TargetHost(""), 9000, false), true, None, None, None, None, Nil, None)
