package com.cloudentity.edge.domain.openapi

import com.cloudentity.edge.domain.flow._
import com.cloudentity.edge.domain.http.RelativeUri
...
case class OpenApiRule(method: HttpMethod, serviceId: ServiceId, group: GroupMatchCriteria, pathPattern: PathPattern, pathPrefix: PathPrefix,
                       dropPathPrefix: Boolean, rewriteMethod: Option[RewriteMethod], rewritePath: Option[RewritePath], plugins: List[PluginConf], tags: List[String]) {
