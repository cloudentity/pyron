...
trait BruteForceCache {
  ...
  @VertxEndpoint def set(ctx: TracingContext, counter: String, identifier: String, attempts: List[Attempt], ttl: Duration): VxFuture[Unit]
  @VertxEndpoint def get(ctx: TracingContext, counter: String, identifier: String): VxFuture[Option[List[Attempt]]]
