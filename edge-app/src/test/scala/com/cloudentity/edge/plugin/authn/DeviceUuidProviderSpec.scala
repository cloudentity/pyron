...
import com.cloudentity.edge.plugin.impl.authn.entities.DeviceUuidProvider
...
class DeviceUuidProviderSpec extends WordSpec with MustMatchers with FutureConversions {

  implicit lazy val ec: VertxExecutionContext = VertxExecutionContext(Vertx.vertx().getOrCreateContext())

  val provider = new DeviceUuidProvider

  "DeviceUuidProvider" should {
    "read deviceUuid" in {
      // given
      val uuid = Json.fromString("xyz")
      val ctx = AuthnCtx("deviceUuid" -> uuid)

      // when
      val result = Await.result(provider.getEntity(TracingContext.dummy, ctx).toScala, 1 second)

      //then
      result mustBe(AuthnCtx("deviceUuid" -> uuid))
    }

    "fail when deviceUuid missing in ctx" in {
      // given
      val ctx = AuthnCtx()
