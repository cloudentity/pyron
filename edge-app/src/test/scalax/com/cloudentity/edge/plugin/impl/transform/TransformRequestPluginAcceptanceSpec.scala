package com.cloudentity.edge.plugin.impl.transform
...
class TransformRequestPluginAcceptanceSpec extends ApiGatewayTest with MustMatchers {
  override def getMetaConfPath(): String = "src/test/resources/transformer/meta-config.json"
  ...
  @Test
  def shouldSetPathParamFromAuthnCtx(): Unit = {
    given()
      .header("userUuid", "123")
    .when()
      .get("/path-param-from-authn/value")
    .`then`()
      .statusCode(200)

    assertTargetRequest { req =>
      req.getPath must be ("/path-param-from-authn/123")
    }
  }
