...
class DeleteBruteForceAttemptsRoute extends ScalaRouteVerticle with RouteService with CirceRouteOperations with BruteForceEvaluator {
  ...
  override protected def handle(ctx: RoutingContext): Unit = {
    val program: Future[ApiError \/ Unit] = {
      for {
        ...
        _           <- BruteForce.clearBruteForceAttempts(cache)(counterName).toOperation
      } yield ()
    }.run
