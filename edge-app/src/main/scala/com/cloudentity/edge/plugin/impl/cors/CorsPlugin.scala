...
import com.cloudentity.edge.domain.rule.{RequestPluginsConf, ResponsePluginsConf, RuleConfWithPlugins}
...
class CorsPlugin extends RequestResponsePluginVerticle[CorsPluginConf] with ConfigDecoder {
  ...
  override def initService(): Unit = {
    if (Objects.isNull(getConfig)) throw new Exception("CorsPlugin configuration is missing")
  ...
  override def extendRules(ruleConf: RuleConfWithPlugins, conf: CorsPluginConf): ExtendRules = {
    ...
    val preRequestPlugin = PluginConf(PluginName("cors"), confEncoder(conf))
